...
console.log(PIXI);
//{ width: 600, height: 900, backgroundColor: 0x1099bb }
const app = new PIXI.Application({
  ...
  stage: { //  корневой контейнер для всех элементов
    x: 100, // координаты
    y: 100, // координаты
    width: 300,
    height: 300,
   ...
   renderer: { // рендерер  всех элементов,  начиная с stage, рекурсивно проходит по всем элементам дочерним и рисует их
  ...
  loader : { //  позволяет асинхронно загружать ресурсы (изображения или аудио файлы) и создавать спрайты можно его не использовать если мы используем графику
  ...
  ticker: { //  работает  requestAnimationFrame()  обновляет состояние приложения перед каждым кадром анимации, чтобы вызвать рендер который отрисует  стейдж
